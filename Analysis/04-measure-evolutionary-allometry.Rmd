...
evo.allo.gen <- procD.pgls(shape ~log(CS) * genus, aus.tree, data = evo.allo.gen.gdf)
evo.allo.gen
...
Given the New World monkey papers (Chervaud & Marriog 2005, 2009), we would expect the carnivores to have the slowest rates of evolution and rodents that evolve along the allometric line to have the greatest rates of evolution. 
...
herb.grps <- rep(1, dim(info.means.tree)[1])
herb.grps[is.sp.herb] <- 2

names(herb.grps) <- info.means.tree$FullName
compare.evol.rates(mean.shapes.tree, aus.tree, herb.grps)
```
Intriguingly, the specialized herbivores evolved more quickly than the rest of the group!

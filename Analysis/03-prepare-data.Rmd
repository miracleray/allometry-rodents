...
pch.spp <- PlotByGroup(info, "Taxa", pch.unique.spp)  # pch coded for each individual specimen
...
# Make color and pch vectors for legends by species
unique.taxa <- info[match(unique(info$FullName), info$FullName), ]
col.unique.spp <- PlotByGroup(unique.taxa, "EGenus", col.gen)
names(col.unique.spp) <- NULL
col.unique.spp <- col.unique.spp[phylo.order]  # colors for legends
pch.unique.spp <- pch.unique.spp[phylo.order]  # points for legends
```
...
# Graphing vectors
save(genera.phylo, col.phylo, col.gen, col.unique.spp, sp.col.gen, phylo.names, pch.spp, col.unique.spp, pch.unique.spp, file = "../Data/Processed/03-graphing-vectors.rda")

...
# Plot PC1 Min/Max
For the heatmap, we use Dr Thomas Guillerme's new package, `landvR` to see how specific landmarks vary along the PC axes. 
...
This step assumes you have ImageMagik installed
```{r}
...
text(dim(dor)[2]-10, dim(dor)[1]-10, "a", cex = 2)

# Plot lateral view with same dimensions as above
plot(c(0, dim(dor)[2]), c(0, dim(dor)[1]), type = "n", axes = F, xlab = "", ylab = "", asp = T)
rasterImage(lat, 0, 0, dim(dor)[2], dim(dor)[1])
text(dim(dor)[2]-10, dim(dor)[1]-10, "b", cex = 2)

# Make gradient legend
legend_image <- as.raster(matrix(heat.colors(20), ncol = 1))
rasterImage(legend_image, 0, 220, 25, 160)
text(70, 210, "High variation", col = "dark grey")
text(50, 172, "Low", col = "dark grey")
dev.off()
```
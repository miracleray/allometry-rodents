---
title: "13-extract-data-for-analyses"
author: "Ariel Marcy"
date: "12/20/2018"
...
load(file = "../Data/Processed/02-main-data-big-protocol.rda")
load(file = "../Data/Processed/02-main-data-sm-protocol.rda")
```

## Set which patch protocol to use as your starting shape and metadata:
```{r}
shape <- sm.shape  # use big.shape to start with the big patch data
info <- sm.info  # use big.info to start with the big patch data
```

# Begin preparing metadata
Handle genus Mastacomys falling within genus Pseudomys in recent phylogenetic analyses
```{r}
# Make an "Effective Genus" column since Mastacomys falls within the genus Pseudomys
info$EGenus <- info$Genus
info$EGenus[which(info$Genus == "Mas")] <- "Pse"

# Do the same for an Effective Taxa column
info$ETaxa <- info$Taxa
info$ETaxa[which(info$Taxa == "M_fus")] <- "P_fus"
```

Combine metadata with additional trait information gathered from the book, Native Mice and Rats by Dr.s Bill Breed and Fred Ford (2007). 
```{r}
info <- info[, c(-5, -7)]  # removes now-unnecessary columns
...
info.traits <- info.traits[order(info.traits$Order), ]  # restore order

# Rename Genus & Species columns so short names have their old col names
names(info.traits)[names(info.traits) == 'Genus.x'] <- 'Genus'
names(info.traits)[names(info.traits) == 'Species.x'] <- 'Species'
```

# Prepare shape and evolutionary tree for evolutionary analyses
...
```{r}
shape.phylo <- shape  # initiate shape ready for mean shape
...
## Include phylogenetic data in metadata
 



## Color schemes for PCA
Color genus

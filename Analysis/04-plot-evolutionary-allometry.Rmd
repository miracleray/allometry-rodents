...
info.means$Csize <- cs.means[, 2]  # store in vector
...
pch.means[which(info.means$Taxa == "M_fus")] <- 2
...
cor <- cor.test(info.means$MeanCsize, pca.means$pc.scores[, 1], method = "pearson")
cor.assoc <- round(unname(cor$estimate), digits = 3)  # round to 3 sig figs
cor.assoc
cor$p.value

# Find Pearson's r for correlation with PC2
cor2 <- cor.test(info.means$MeanCsize, pca.means$pc.scores[, 2], method = "pearson")
cor.assoc2 <- round(unname(cor2$estimate), digits = 3)  # round to 3 sig figs
cor.assoc2
cor2$p.value
```
...
# Call out specialized herbivores with special characters
pch.evo.allo <- rep(16, length(col.means))
pch.evo.allo[which(info.means$Taxa == "P_ora")] <- 11
pch.evo.allo[which(info.means$Taxa == "M_fus")] <- 2
...
PCx.lab <- paste("PC", 1, " (", PCx.per, "%)", sep = "")
PCy.per <- round(PCs[2, 2] * 100, digits = 1)
PCy.lab <- paste("PC", 2, " (", PCy.per, "%)", sep = "")
```

## Create multi-panel figure
PCA on top with Reg.proj plot on the bottom
```{r}
layout(matrix(c(1,1,2,2), 2, 2, byrow = TRUE))  # 2 rows, 1 fig on top
...
plot(x = log(info.means$MeanCsize),
     ...
     ylab = "Projected regression score")
legend(6, 0.135, legend = names.phylo, col = col.phylo, pch = 16, cex = 0.6, pt.cex = 1, ncol = 2)

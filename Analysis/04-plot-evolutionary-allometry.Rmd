...
# Plot evolutionary allometry and PC1 heatmap
Here we plot the PCA and evolutionary allometry plots as well as the PC1 min and max heatmap showing what shape changes occur. PC1 is highly correlated allometric shape. 
...
## Make multipanel figure: mean PCA, evolutionary allometry, and PC1 heatmap
...
PC1.lab <- paste("PC", 1, " (", PCx.per, "%)", sep = "")
PC2.per <- round(PCs[2, 2] * 100, digits = 1)
PC2.lab <- paste("PC", 2, " (", PCy.per, "%)", sep = "")
...
### Plot PC1 Min/Max
For the heatmap, we use Dr Thomas Guillerme's new package, `landvR` to see how specific landmarks vary along the PC axes. Much of the below was adapted from [this vignette written by Dr Guillerme](https://cdn.rawgit.com/TGuillerme/landvR/8a6a6bd5/inst/vignettes/Landmark_partition_test.html).
```{r}
...
rgl.snapshot("PC1heatmap.png")  # save screenshot for multipanel plot

```

### Plot and export the multi-panel figure
...
plot(x = -pca.means$pc.scores[, 1],
     ...
     xlab = PC1.lab, 
     ylab = PC2.lab)
text(.105, -0.003, "Frugivores", col = "darkgoldenrod4")
draw.ellipse(.116, .015, .006, .012, angle = -12, border = "darkgoldenrod4")

text(.075, 0.041, "Herbivores", col = "dark blue")
draw.ellipse(.048, .033, .025, .007, angle = -19, border = "dark blue")

text(.045, -0.054, "Carnivores", col = "red")
draw.ellipse(.044, -.073, .034, .007, angle = -15, border = "red")

text(-.055, 0.035, "Notomys", col = "blue")
draw.ellipse(c(-.053, -0.07), c(.014, -0.013), c(.015, .004), c(.004, .007), angle = c(-95, 0), border = c("blue", "blue"))  # draws both
...
text(5.76, 0.11, "Frugivores", col = "darkgoldenrod4")
text(5.35, 0.053, "Herbivores", col = "dark blue")
text(5.81, 0.075, "Carnivore", col = "red")
text(5.03, 0.02, "Carnivore", col = "orange")
text(5.3, -0.065, "Hopping Notomys", col = "blue")

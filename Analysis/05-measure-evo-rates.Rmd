...
## Rates calculated all together
```{r}
# Define groups; 1 = those on allometric line / average body size 
groups <- rep(1, dim(info.means)[1])

is.frug <- c(which(info.means$Taxa == "U_cau", info.means$Taxa == "M_gou"))
groups[is.frug] <- 2  # Define frugivores as 2

is.fol <- c(which(info.means$Taxa == "P_ora"), which(info.means$Taxa == "M_fus"), which(info.means$Taxa == "L_con"))
...
groups[which(info.means$Genus == "Not")] <- 5  # Define Notomys as 5
...
specialist.test$pairwise.pvalue
specialist.test$sigma.d.gp.ratio

p.adjust(specialist.test$pairwise.pvalue, method = "bonferroni")
```
...
expected <- c("Slower", "Slower", "Slower", "Slower")

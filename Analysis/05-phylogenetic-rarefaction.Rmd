...
par(mar = c(1, 4, 1.5, 1))
boxplot(delta.slopes, border = "white", ylim = quantile(unlist(delta.slopes), probs = c(0.01, 0.99), na.rm = TRUE), ylab = expression(paste(Delta, " slope")), xlab = NULL, names = rep("", length(new.names)))  # an empty plot needed first
...
par(mar = c(5, 4, 0, 1))
plot(c(1:length(med.delta.slopes)), angle.changes, xlab = "Clade (n species in clade and/or species name)", ylab = expression(paste("Median ", Delta, " angle (\u00B0)")), pch = 16, ylim = c(0,90), xaxt = 'n', cex = 0.5)

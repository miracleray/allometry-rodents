...
static.allo.rs <- plotAllometry(fit.static, size = static.gdf$size, method = "RegScore")  # Allometry data
...
clade.names <- unlist(lapply(clades, paste, collapse = "+"))
new.names <- clade.names
...
new.names[1] <- paste("All", length(new.names))
...
## Save data that takes a long time to generate
```{r}
save(delta.slopes, angle.changes, f.delta.slopes, fake.results.list, file = "../Data/Processed/05-Phylo-rare-results.rda")
```
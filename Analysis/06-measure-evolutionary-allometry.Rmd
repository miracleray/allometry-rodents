...
```{r}
physignal(mean.shapes.tree, aus.tree, iter = 499)
```
...
evo.allo.gen <- procD.pgls(Shape ~log(CS) * Genus, aus.tree, iter = 499, data = evo.allo.gen.gdf)
evo.allo.gen
```
...
```{r}
save(mean.shapes.tree, info.means.tree, aus.tree, renamed.tree, file = "../Data/Processed/06-phylo-data.rda")
```
...
First, we visualize whether any outliers can be explained by mistakes in landmark placement (e.g. a landmark "falling through" the nasal to land on the inner basicranium).
...
```
**Repeat the above for each genus/patch protocol as needed. As landmarking errors are found, fix in Viewbox, export new coordinates, and begin again from script 01.** 
...
```{r}
save(big.shape, big.info, file = "../Data/Processed/02-main-data-big-protocol.rda")
save(sm.shape, sm.info, file = "../Data/Processed/02-main-data-sm-protocol.rda")
```
...
TestHOS <- function(shape, size, group, iter = 500, lm.fun) {
        ...
        fit.common <- lm.fun(coords ~ log(size) + spp, iter = iter, data = spp.gdf, RRPP = FALSE, print.progress = FALSE)  # common allometries across spp (NULL HYPOTHESIS)
        #anova(fit.unique, fit.common, print.progress = TRUE)

        return(pairwise(fit.unique, fit.common, groups = spp.gdf$spp, print.progress = FALSE))  # includes Homogeneity of Slope pairwise comparisons
}
...
pw.summary <- summary.pairwise(pw.comp, test.type = "VC", angle.type = "deg", confidence = 0.99, show.vectors = FALSE)  
actual.pw <- length(which(round(pw.summary$pairwise.tables$P, 3) < 0.01))

# Export p-value matrix to Results folder
write.csv(round(pw.summary$pairwise.tables$P, 3), "../Data/Results/TableS4_HOS_all.csv")
```

## 1b) Randomize specimens to get NULL expantancy for pairwise significant results
We'll do the randomization procedure 100 times. This takes at least 2.5 hours to run.
```{r}
...
set.seed(42)
count.pw <- NULL
for(i in 1:100){   # 100 iterations takes ~2.5 hours
...
abline(lm(sa.mean.shape$V1 ~ log(sa.mean.size$x)), col = "dark grey", lwd = 2)  # plots evolutionary allometry
...
is.parallel <- test.delta.slope(obs.slopes, med.delta.slopes, significance = 4.5) # 4.5 ideal cut-off because 90*0.5 = 4.5, therefore has 5% chance of being off so good null; 9 degrees is not horrible (10% chance)
sum(is.parallel)

# Plot angle changes in degrees
denom <- (1 + obs.slopes * (obs.slopes + med.delta.slopes))
angle.changes <- abs(atan(med.delta.slopes/denom) * 180/pi)  # angles in deg

plot(c(1:length(med.delta.slopes)), angle.changes, xlab = "Clade", ylab = "Median delta slope (deg)", pch = 16, ylim = c(0,90))
abline(h = 4.5, col = "coral4")  # significance level
text(1, 7, "4.5 degree signifcance level", adj = c(0,0), col = "coral4")
```
...
test.delta.slope <- function(slope, significance) {
    angle <- abs(atan(slope) * 180/pi)  # converts angle into degrees
    if(!missing(significance)) {
        return(ifelse(angle < significance, TRUE, FALSE))  # FALSE = failed the signifance level
    } else {
        return(angle)
...
pw.comp.2 <- pairwise(fit.unique.2, fit.common.2, groups = spp.rdf$spp, print.progress = FALSE)

pw.summary.2 <- summary.pairwise(pw.comp.2, test.type = "VC", angle.type = "deg", confidence = 0.95, show.vectors = TRUE)  
actual.pw.2 <- length(which(pw.summary.2$pairwise.tables$P < 0.01))

...
rarefy.stat <- function(data, stat.fun, rarefaction, replicates = 100, observed, ...) {
    ...
    if(!is.null(rarefaction)) {
        ...
        if(rarefaction > nrow.data) {
            warning(paste0("Rarefaction value (", rarefaction, ") is bigger than the number of rows in the data (", nrow.data, ").\nThe statistic will be only bootstrapped."))
    ...
    results <- replicate(replicates, stat.fun(data[sample(1:nrow.data, rarefaction, replace = TRUE), ]))
...
delta.slopes <- lapply(data.clades, rarefy.stat, stat.fun = GetSlope, rarefaction = 5)
```
...
```{r}
# Getting the rarefied statistics for each clade
delta.slopes.overall <- lapply(data.clades, rarefy.stat, stat.fun = GetSlope, rarefaction = 5, observed = GetSlope(rare.table[, c(2, 3)]))

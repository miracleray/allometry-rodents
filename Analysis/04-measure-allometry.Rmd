...
### 2b) Measure Evolutionary Allometry using only clades that have 2 or more spp -- Supplementary Table S5 (n = 35 species mean shapes, 5 clades)
...
pgls.gdf <- geomorph.data.frame(coords = e.mean.shapes, size = e.info.means$MeanCsize, clade = e.info.means$Clade)

fit.pgls <- procD.pgls(coords ~ log(size) * clade, e.aus.tree, iter = 500, data = pgls.gdf, print.progress = FALSE)
anova(fit.pgls)

# Round all values in ANCOVA table to 3 sig figs & remove NAs from the table
table1b_n35 <- as.data.frame(round(evo.allo.clade$aov.table, 3))
table1b_n35[is.na(table1b_n35)] <- ""

write.table(table1b_n35, "../Data/Results/TableS5_ANOVA_pgls.csv", sep = ",", col.names = NA)
```
...
par(mar = c(1, 4, 1.5, 1))
boxplot(delta.slopes, border = "white", ylim = quantile(unlist(delta.slopes), probs = c(0.01, 0.99), na.rm = TRUE), ylab = "Delta slope", xlab = NULL, names = rep("", 73))  # an empty plot needed first
...
colnames(fake.results.table) <- n.spp.group
plot(NULL, xlim = c(0, length(fake.results.list)), ylim = quantile(unlist(fake.results.table), probs = c(0.01, 0.99), na.rm = TRUE))

boxplot(fake.results.table, border = "white", ylim = quantile(unlist(fake.results.table), probs = c(0.01, 0.99), na.rm = TRUE), ylab = "Delta slope", xlab = "Number of species")

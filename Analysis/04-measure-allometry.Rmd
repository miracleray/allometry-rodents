...
TestHOS <- function(shape, size, group, iter = 500, lm.fun) {
        ...
        # Models
        fit.unique <- lm.fun(coords ~ log(size) * spp, iter = iter, data = spp.gdf, print.progress = FALSE)  # unique allometries by spp
        fit.common <- lm.fun(coords ~ log(size) + spp, iter = iter, data = spp.gdf, print.progress = FALSE)  # common allometries across spp
...
new.names <- names(delta_slopes)
for(i in 1:length(names(delta_slopes)))
    if(nchar(new.names[i]) > 5) {  # detect bins of greater than 1 spp
        new.names[i] <- ((nchar(names(delta_slopes[i])) + 1) / 6)  # gets number of spp
    }
...
```{r}
# An empty plot first
boxplot(delta.slopes.data, border = "white", ylim = quantile(unlist(delta.slopes.data), probs = c(0.01, 0.99), na.rm = TRUE), ylab = "Delta slope", xlab = "Clade", names = new.names, las = 2)

...
is.parallel <- test.delta.slope(med.delta.slopes, significance = 9) # 4.5 ideal cut-off because 90*0.5 = 4.5, therefore has 5% chance of being off so good null; 9 degrees is not horrible (10% chance)
sum(is.parallel)

angle.changes <- abs(atan(med.delta.slopes) * 180/pi)

plot(c(1:length(med.delta.slopes)), angle.changes, xlab = "Clade", ylab = "Delta slope angle (deg)", pch = 16)
```

...
first.node <- length(relabel.tree$tip.label) + 1
relabel.tree$node.label <- rates[first.node:length(rates)]  # grab node rates; length(node.label) should = Nnode

concord <- name.check(relabel.tree, relabel.tree$tip.label, dimnames(mean.shapes.tree)[[3]])  # check concordance
aus.tree <- drop.tip(relabel.tree, concord$tree_not_data)  # prune tree to Aus sample
```

Tip rates
```{r}
aus.rates <- rates[which(!is.element(relabel.tree$tip.label, concord$tree_not_data))]
...
plotBranchbyTrait(aus.tree, as.numeric(aus.rates), palette = "gray", tip.color = col.tree, mode = "nodes")

plot.phylo(aus.tree, edge.color = gray.branches, tip.color = col.tree, no.margin = T); edgelabels(cex=0.5)
```
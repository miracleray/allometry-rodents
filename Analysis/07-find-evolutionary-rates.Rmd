...
```{r}
p.values.bon <- p.adjust(c(frug.test$P.value, herb.test$P.value, carn.test$P.value, not.test$P.value), method = "hommel")
```

## Create and export Table 2
```{r}
# Evolutionary rate ratio to 1 decimal place
ratio <- paste("1:", round(c(frug.test$sigma.d.ratio, herb.test$sigma.d.ratio, not.test$sigma.d.ratio, carn.test$sigma.d.ratio), 1), sep = "")
...
```{r}
# Combined tip and node rates
combined.rates <- c(aus.rates, aus.tree$node.label)
...
### BIN
### Find relative evolutionary rates for each taxa
```{r}
# Set up new metadata table
evol.rates <- info.means.tree[, c(1:3, 9:11)]  
evol.rates$ratio <- "NA"
evol.rates$P.value <- "NA"
evol.rates$rel.rate <- "NA"

# Run for loop to find evolutionary rates for each taxa (~22 min)
for(i in 1:length(evol.rates$Taxa)){
        taxa.grp <- rep(2, dim(info.means.tree)[1])
        taxa.grp[i] <- 1  # taxa of interest
        names(taxa.grp) <- evol.rates$FullName
        
        # Run evolutionary rate test and store results
        taxa.test <- compare.evol.rates(mean.shapes.tree, aus.tree, taxa.grp, iter = 499)
        evol.rates$ratio[i] <- round(taxa.test$sigma.d.ratio, 3)
        evol.rates$P.value[i] <- taxa.test$P.value
        evol.rates$rel.rate[i] <- FindRateDiff(taxa.test)
}

# Adjust table to account for p value results
evol.rates$cor.P.val <- p.adjust(evol.rates$P.value, "hommel")  # correct for multiple comparisons
evol.rates$rel.rate[which(evol.rates$cor.P.val > 0.05)] <- paste("(", evol.rates$rel.rate, ")", sep = "")  # ID non-significant results
evol.rates$rel.rate[which(evol.rates$P.value < 0.05)] <- paste(evol.rates$rel.rate, "*", sep = "")  # ID marginal results
```

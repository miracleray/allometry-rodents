...
ERROR: memory issues when trying to add legend within setEPS. Works when lines inbetween `postscript()` and `dev.off()` are run through the console, plot is "zoomed" from RStudio, and then screenshot from that window
```{r}
...
legend(x = "bottomright", legend = genera.phylo, col = col.phylo, pch = 16, cex = 0.85, pt.cex = 0.85, ncol = 3, bty = "n")
...
text(-.067, 0.027, "Notomys", col = Not.col)

...
Run allometry tests as well as a homogeneity of slopes test. These tests may take 1-2 min to run. Updated for `geomorph` 3.1.0
```{r}
# ANOVA to test for common static allometries by species
spp.gdf <- geomorph.data.frame(coords = shape.HOS, size = info.HOS$Csize, spp = info.HOS$Taxa)  # data frame

fit.unique <- procD.lm(coords ~ log(size) * spp, iter = 500, data = spp.gdf, print.progress = FALSE)  # unique allometries by species
fit.common <- procD.lm(coords ~ log(size) + spp, iter = 500, data = spp.gdf, print.progress = FALSE)  # common allometries across species

anova(fit.common, fit.unique, print.progress = FALSE)

# Homogeneity of Slope pairwise comparisons
pw.comp <- pairwise(fit.common, fit.unique, groups = spp.gdf$spp, print.progress = FALSE)
pw.summary <- summary.pairwise(pw.comp, test.type = "VC", angle.type = "deg", confidence = 0.95, show.vectors = TRUE)  
length(which(pw.summary.2$pairwise.tables$P < 0.05))  # quick count of significant pw differences
...
WORK IN PROGRESS
```{r}
# Define function to shuffle species attributions
null.pairwise <- function() {
        rand.spp <- sample(info.HOS$Taxa)
        info.HOS$randTaxa <- rand.spp
        
        # Run the test

        
        # Count the number of significants
        return(length(which(pwHOS.spp$P.slopes.dist < 0.01)))
}

null_results <- replicate(100, null.pairwise())

hist(null_results)
abline(v = pwHOS.spp$P.slopes.dist, lwd = 3)
...
# Do 100 times
## Make the list of fake slopes into matrix
fake.slope.matrix <- lapply(RareFlakes, function(x) do.call(cbind, x = 100))
all.fake.slopes <- do.call(cbind, fake.slope.matrix) # Combining all these matrices
```
...
names(delta.slopes.overall) <- unlist(lapply(fake.clades, paste, collapse = "+"))
```
...
boxplot(do.call(cbind, delta.slopes),
        ylab = expression(paste(Delta, "slope")), xlab = "Spp in Clade",
        main = "Slope difference for 100 FAKE CLADEs", ylim = c(-100, 100), las = 2)
abline(h = 0, lty = 3)

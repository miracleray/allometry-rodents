...
cor.static.assoc <- round(unname(cor.static$estimate), digits = 3)  # round to 3 sig figs
cor.static.assoc
cor.static$p.value

# Plot static and evolutionary allometry
...
abline(lm(mean.shape$V1 ~ log(mean.size$x)))  # plots evolutionary allometry
legend(6.04, 0.06, legend = sort(unique(info$Taxa)), col = col.unique.spp, border = NULL, pch = pch.unique.spp, pt.bg = col.unique.spp, cex = 0.7, pt.cex = 0.8, ncol = 2, x.intersp = .3, y.intersp = 0.6)
```

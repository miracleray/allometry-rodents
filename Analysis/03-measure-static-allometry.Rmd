...
pw.summary.2 <- summary.pairwise(pw.comp, test.type = "dist", angle.type = "deg", confidence = 0.99, show.vectors = TRUE)  # alpha = 0.01
length(which(pw.summary.2$pairwise.tables$P < 0.05))  # quick count of significant
...
fake.clades <- lapply(n.spp.group, function(n.spp.group, names = rare.table$FullName) sample(names, n.spp.group))   # fake clades
...
### Applying the tests to the "fake clades" (called data.flakes)
...
```{r}
# Adding the names to the slopes
names(delta.slopes) <- unlist(lapply(fake.clades, paste, collapse = "+"))
...
boxplot(do.call(cbind, delta.slopes),
        ylab = expression(paste(Delta, "slope")), xlab = "Clade",
        main = "Slope difference for each FAKE CLADE", ylim = c(-100, 100), las = 2)
abline(h = 0, lty = 3)
boxplot(do.call(cbind, delta.slopes.overall),
        ylab = expression(paste(Delta, "slope")), xlab = "Clade",
        main = "Slope difference for each FAKE CLADE (compared to the whole dataset)", ylim = c(-350, 125), las = 2)
abline(h = 0, lty = 3)  # since observered minus expected should be 0
```
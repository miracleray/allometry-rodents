...
```{r}
plot(x = log(info$Csize), 
     y = PCA$pc.scores[, 1], 
     col = sp.col.gen, 
     pch = 16, 
     xlab = "Log Centroid size", 
     ylab = "PC1 score", 
     main = "Static Allometry: PC1 vs Log Centroid Size")
legend(415, 0.16, legend = names.phylo, col = col.phylo, pch = 16, cex = 0.66)
text(5.7, 0, paste("r =", cor.assoc), col = "dark grey")
```
...
# Find pair-wise p-values for slopes for each taxa, code written with Dr Emma Sherratt
size <- info$Csize
spp <- info$Taxa
...
pwHOS.pvals <- p.adjust(pwHOS.spp$P.slopes.dist, method = "BH")  # Hommel is a more powerful method for independent hypothesis tests

# Export p-values
write.csv(pwHOS.pvals, "../Data/Results/19_01_24_static_pwHOS.csv")  # save in Results folder
```
...
plot(x = log(spp.results$size),
     ...
     pch = pch.spp,
     bg = sp.col.gen,
     main = "Predicted Allometric Shape by Species",
     xlab = "Log centroid size", 
     ylab = "Predicted shape (CAC)")
text(4.75, 0.03, "Mus", col = "grey")
text(5.65, 0.042, "Rattus")
# legend(6.03, 0.051, legend = info.means$Taxa, col = col.means, border = NULL, pch = pch.means, pt.bg = col.means, cex = 0.42, ncol = 2)
```
...
plot(x = log(gen.results$size),
     ...
     xlab = "Log centroid size", 
     ylab = "Predicted shape (CAC)")
legend(6.04, 0.12, legend = names.phylo, col = col.phylo, pch = 16, cex = 0.6)
```

## Create multi-panel figure
CAC plot on top with PCA and PC1 vs CS on the bottom
```{r}
layout(matrix(c(1,1,2,3), 2, 2, byrow = TRUE))  # 2 rows, 1 fig on top
...
plot(x = spp.results$size,
     ...
     col = sp.col.gen, 
     pch = 16, 
     main = "Static Allometry: Predicted Allometric Shape by Genus",
     xlab = "Log centroid size", 
     ylab = "Predicted shape (CAC)")  # figure on top
legend(408, 0.055, legend = names.phylo, col = col.phylo, pch = 16, cex = 1, pt.cex = 1, y.intersp = 0.25)
text(105, 0.04, "A", cex = 1.35)

PlotPCA(PCA, 1, 2, sp.col.gen)  # first figure on bottom
text(-.10, 0.053, "B", cex = 1.35)
...
plot(x = log(info$Csize), 
     ...
     xlab = "Log Centroid size", 
     ylab = "PC1 score",
     main = "PC1 to Size Correlation")  # 2nd fig on bottom
legend(415, 0.16, legend = names.phylo, col = col.phylo, pch = 16, cex = 1)
text(5.7, 0, paste("r =", cor.assoc), col = "dark grey")
text(4.77, 0.12, "C", cex = 1.35)
```

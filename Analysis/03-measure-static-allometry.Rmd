...
# Species Legend
legend(6.01, 0.06, legend = sort(unique(info$Taxa)), col = col.unique.spp, border = NULL, pch = pch.unique.spp, pt.bg = col.unique.spp, cex = 0.75, pt.cex = 0.8, ncol = 2, x.intersp = .3, y.intersp = 0.6)
...
plot(x = log(spp.results$size),
     ...
     ylab = "Shape (Proj. Reg. Score)")
abline(lm(mean.shape$V1 ~ log(mean.size$x)), col = "dark grey", lwd = 2)  # plots evolutionary allometry
text(6, -0.02, paste("r = ", cor.static.assoc), cex = 1.3, col = "dark grey")
